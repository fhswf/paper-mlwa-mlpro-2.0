...
from mlpro.oa.streams.tasks import BoundaryDetector, Normalizer, NormalizerZTransform
...
## -------------------------------------------------------------------------------------------------
    def _setup(self, p_mode, p_ada: bool, p_visualize: bool, p_logging):
        ...
        # 1 Load the FD001 dataset from C-MAPSS
        stream = StreamMLProCSV( p_path_load = os.path.join( os.path.dirname(__file__), 'C-MAPSS Dataset'), 
                                 p_csv_filename = 'train_FD001_unit_001.csv',
                                 p_frame = False,
                                 p_list_features = ['s_9', 's_12'] )


        # 2 Set up a stream workflow based on a custom stream task

        # 2.1 Creation of a workflow
        workflow = OAStreamWorkflow( p_name = 'Input signal: C-MAPSS FD001/U001',
        ...
        # 2.3 Add a Z-Trans Normalizer and connect to the boundary detector
        task_norm_ztrans = NormalizerZTransform( p_name = 'T2 - Z-transformation', 
                                                 p_ada = p_ada, 
                                                 p_duplicate_data = True,
                                                 p_visualize = p_visualize, 
                                                 p_logging = p_logging )
        ...
        task_ma = MovingAverage( p_name = 'T3 - Moving average (renormalized)', 
                                 p_ada = p_ada,
                                 p_visualize = p_visualize,
                                 p_logging = p_logging,
                                 p_centroid_crosshair_labels = False )
        
        workflow.add_task( p_task = task_ma, p_pred_tasks = [ task_norm_ztrans ] )
        task_norm_ztrans.register_event_handler( p_event_id = NormalizerZTransform.C_EVENT_ADAPTED, p_event_handler = task_ma.renormalize_on_event )
